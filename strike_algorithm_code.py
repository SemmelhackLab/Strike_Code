...
def init_condition(avis):
    ...
    hsv[..., 1] = 255
    directions_map = np.zeros([5, 5])

    return capture, num_frames, prvs, hsv, directions_map


def calculate_movement(capture, num_frames, prvs, hsv, directions_map):
    mag1 = []
    ang1 = []
    strike_frame = []
    mag_final = []
    ...
    for k in range(0, len(res)):
        ...
        if res[k] < (num_frames) - 1:
            ...
            elif 330 < move_mode or move_mode < 10:
                   loc=4
            else:
                    loc=5
...
def plot(mag_final, arg_final, strike_frame, filename):
    ...
    # ax1.set_ylabel('Average Movement(in Pixels)')
    # ax1.set_xlabel('Frame Number')
    # ax1.plot(mag_final)
    #
    # ax2 = fig.add_axes([0.15, 0.1, 0.7, 0.3])
    # ax2.plot(arg_final)
    # ax2.set_ylabel('Average Movement (in degrees)')
    ...
    for a in range(0, len(list_folder)):
        ...
        for k in range(0,len(avis)):
         if len(avis) != 0:
            ...
            capture, num_frames, prvs, hsv, directions_map = init_condition(folder + '\\' + avis[k])
            filename1 = folder + '\\' + avis[k]
            filename = filename1.replace('.avi', "")

            print(filename1)
            mag_final, arg_final, strike_frame = calculate_movement(capture, num_frames, prvs, hsv, directions_map)
